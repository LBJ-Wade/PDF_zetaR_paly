...
public:
  JacobiPDE(){}
  // Params[0] = maxstep, Params[1] = tol, Params[2] = funcNo
  JacobiPDE(vector< vector< vector<double> > > &Site, vector<double> &It, vector<double> &Params);
  ...
  virtual double DD(vector< vector<double> > &psv, int func);
  ...
  /*
    solve PDE :
    (DD(xp) + DI(xp,I) \partial_xpI + 1./2 DIJ(xpI,xpJ) \partial_xpI \partial_xpJ) f = CC
  */
